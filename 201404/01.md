...
上周写了一篇 [文章](https://github.com/fouber/blog/issues/1) 介绍前端集成解决方案方案的基本理论，很多同学看过之后大呼不过瘾。

> 干货 ~~fuck things~~ 在哪里！

本打算继续完善理论链，形成前端工程领域知识结构。但鉴于如今的快餐文化，po主决定还是先写一篇实战介绍，让大家看到前端工程体系能为团队带来哪些好处，调起大家的胃口再说。

> ps: 写完才发现这篇文章真的非常非常长，涵盖了前端开发中的很多方面，希望大家能有耐心看完，相信一定会有所斩获。。。

## 2014年02月12日 - 晴

新到松鼠团队的第二天，小伙伴 [@nino](https://github.com/ninozhang) 找到我，说

> nino: 视频项目打算重新梳理一下，希望能引入新的技术体系，解决现有的一些问题。

po主不禁暗喜，好机会啊，这是我专业啊，蓝翔技校·前端集成解决方案学院·自动化系·打包学专业 的文凭不是白给的，于是自信满满的对nino说，有什么需求尽管提！

> nino: 我的需求并不多，就这么几条~~

1. 模块化开发。最好能像写nodejs一样写js，很舒服。css最好也能来个模块化管理！
1. 性能要好。模块那么多，可得有按需加载啊，请求不能太多。
1. 组件化开发。一个组件的js、css、模板最好都在一个目录维护，改起来方便。
1. 用 [handlebars](http://handlebarsjs.com/) 作为前端模板引擎。我看这个模板引擎不错，logic-less（轻逻辑）。
1. 用 [stylus](http://learnboost.github.io/stylus/) 写css看起来不错，给我整一个。
1. 图片base64嵌入。有些小图可能要以base64的形式嵌入到页面或者css中。对了，嵌入之前记得压缩图片啊。
1. js/css/图片压缩没问题吧。
1. 与我们的ci平台集成也简单吧。对了，图片压缩最好别依赖什么系统库，ci的机器未必有。
1. 开发体验要好。文件监听，再配合个浏览器自动刷新([livereload](http://www.livereload.com/))就更好了。
1. 我们用nodejs作为服务器，本地要能预览哟，最好再能抓取线上数据，方便调试。
...
这样历史浏览器浏览历史版本，没事还能做个灰度功能啥的，多好!

此外，我们将来会有多个项目，希望能共用一些组件或者模块，也会
公布一些模块给第三方使用，所以这个功能也要加上。
```
...
我凝望着会议室白板上的这些需求，正打算争辩什么，一扭头发现nino已经不见了。。。正在沮丧之际，小伙伴 [@hinc](https://github.com/hinc) 过来找我，跟他大概讲了一下nino的需求，正想抱怨工期问题时，hinc说

> hinc: 恩，这些正是我们需要的需求，不过我这里还有一个需求
...
这样一套规范、框架、工具和仓库的开发体系，服从我之前介绍的 [前端集成解决方案](https://github.com/fouber/blog/issues/1) 的描述。这样的系统前端界每天都团队在设计和实现，它们其实是有规律可循的。而百度出品的 [fis](https://github.com/fis-dev/fis/wiki) 就是一个能帮助快速搭建前端集成解决方案的工具。使用fis我应该可以在3天之内完成这些任务。

> ps: 这不是一篇关于fis的软文，如果这样的一套系统在grunt构建相信会有非常大量的开发工作，3天完成几乎是不可能的任务。

不幸的是，现在fis官网所介绍的 **并不是** fis，而是一个叫 [fis-plus](https://github.com/fex-team/fis-plus) 的项目，该项目并不像字面理解的那样是fis的加强版，而是在fis的基础上定制的一套完整的解决方案，以php为后端语言，跟smarty有较强的绑定关系，有着 ``19项`` 技术要素，密切配合百度现行技术选型。绝大多数非百度前端团队都很难接受这19项技术选型。尤其是其中的部署、框架规范，跟百度前端团队相关开发规范、部署规范、以及php、smarty等有着较深的绑定关系。

因此如果你的团队用的不是 ``php后端`` && ``smarty模板`` && ``modjs模块化框架`` && ``bingo框架`` 的话，请查看 [fis的文档](https://github.com/fis-dev/fis/wiki)，或许不会有那么多困惑。

> ps: 其实fis已经慢慢形成一个解决方案集，很多团队基于fis实践了自己的前端集成解决方案方案。

言归正传，让我们基于 [fis](https://github.com/fis-dev/fis/) 开始实践这套开发体系吧！

### 0. 开发概念定义

前端开发体系设计第一步要定义一下开发概念。开发概念是指针对开发资源的分类概念。开发概念的确立，直接影响到规范的定制。比如，传统的开发概念一般是按照文件类型划分的，所以传统前端项目会有这样的目录结构：
...
这样确实很直接，任何智力健全的人都知道每个文件该放在哪里。但是这样的开发概念划分将给项目维护带来较高成本，并为项目臃肿埋下了隐患，理由是：
...
> ps: 开发概念越简单越好，前面提到的fis-plus也有类似的开发概念，有组件或模块（widget），页面（page），测试数据（test），非模块化静态资源（statis）。有的团队在模块之中又划分出api模块和ui模块两种概念。

### 1. 开发目录设计

基于开发概念的确立，接下来要开始确定目录规范了。我通常会给开发资源的分类目录取一个有语义的名字，三种资源我们可以按照概念直接定义目录结构为：
...
这样划分目录确实直观，但结合前面hinc说过的，希望能结合component，因此我决定将模块化资源目录命名为``components``，得到：
...
而nino又提到过模块资源分为项目模块和公共模块，以及hinc提到过的要从component安装一些公共组件到项目中，因此，一个components目录还不够，想到nodejs用node_modules作为模块安装目录，因此我们的规范中又追加了一个 ``component_modules`` 目录，得到：

    project
      - component_modules    存放公共模块资源
      - components           存放项目模块资源
      - pages                存放页面资源
      - static               存放非模块化资源

前面nino还提到了今后大多数项目采用nodejs作为后端，express是比较常用的nodejs的server框架，它通常会把页面放到 ``views`` 目录下，而静态资源放到 ``public`` 下。因此，为了迎合这样的需求，我将page、static两个目录调整为 ``views`` 和 ``public``，又修改为：
...
考虑到页面也是一种静态资源，而``public``这个名字不具有语义性，与其他目录都有冲突，还不如将其与``views``目录合并，views目录负责存放页面和非模块化资源比较合适，因此最终得到的开发目录结构为：
...
托nino的福，咱们的部署结构将会非常复杂，根据要求，一个完整的部署结果应该是这样的目录结构：
...
突然想到咱们还要有一些可以被第三方使用的模块，public下只有项目名的部署还不够，应改把模块化文件单独发布出来，得到这样的部署结构：
...
由于 ``component_modules`` 这个名字太长了，如果部署后这样写，url会很长，这样也是一个优化点，因此最终决定部署结构为：
...
并不是所有团队都会有这么复杂的部署要求，但这样的案例也并不唯一。每个团队都会有自己的运维需求，前端资源部署经常牵连到公司技术架构问题，因此很多前端项目的开发目录结构会和部署要求保持一致。这也为项目模块的共享带来了成本，因为代码中写的url通常是部署后的路径，迁移之后就可能失效了。
...
``fis-conf.js``是fis工具的配置文件，接下来我们就要在这里进行工具配置了。虽然开发规范和部署规范十分复杂，但好在fis有一个非常强大的 [roadmap.path](https://github.com/fex-team/fis/wiki/%E9%85%8D%E7%BD%AEAPI#roadmappath) 功能，专门用于分类文件、调整文件发布、指定文件的各种属性。

> 所谓构建，其核心任务就是将文件安装某种规则（以文件后缀分类，以模块化/非模块化分类，以前端/后端代码分类）进行分类，然后针对不同文件做不同的构建处理。
...
在fis中，roadmap.pah是一个数组数据，数组每个元素是一个对象，必须包含一个 ``reg`` 属性，取值可以使路径统配字符串或者正则。fis有一个内部的文件系统，会给每个源码文件创建一个fis.File对象，创建对象时，按照roadmap.path的配置逐个匹配文件路径，命中则把除reg之外的其他属性赋值给file对象，fis中各种插件都会读取文件对象的属性值，而不会自己创造规范。有关roadmap.path的工作原理可以看[这里](https://github.com/fex-team/fis-kernel/blob/master/lib/file.js#L156) 以及 [这里](https://github.com/fex-team/fis-kernel/blob/master/lib/uri.js#L48-L70)。
...
fis.config.set('roadmap.path', [
    ...
    {
        reg : /^\/views\/(.*)$/i,          //正则匹配路径，并且捕获views后面的路径作为分组1
        release : '/public/proj/1.0.0/$1'  //发布到 public/proj/1.0.0/分组1 路径下
    }
]);
```

roadmap.path的第二项数据采用了正则作为匹配，正则可以帮我们捕获到分组信息，在release中引用分组是非常方便的。正则匹配 + 捕获分组，成为目录规范配置的强有力工具：

![roadmap.path](https://raw.githubusercontent.com/fouber/blog/master/assets/roadmap.path.png)

从配置中不难发现，版本号被写到了匹配规则中，这样非常不方便工程师在迭代的过程中升级项目版本。因此我们应该将版本号、项目名称等配置独立出来管理。好在roadmap.path还有读取其他配置的能力，修改上面的配置，我们得到：
...
fis.config.set('roadmap.path', [
    ...
    {
        reg : /^\/views\/(.*)$/i,
        release : '/public/${name}/${version}/$1'  //使用${xxx}引用其他配置项
    }
]);
//项目配置
...
fis的配置系统非常灵活，除了 [文档](https://github.com/fex-team/fis/wiki/%E9%85%8D%E7%BD%AEAPI) 中提到的配置节点，其他配置用户可以随便使用。上述配置的roadmap是系统保留的，而name、version都是用户自己随便指定的。fis系统保留的配置节点只有6个，包括：
...
> ps: fis release会将处理后的结果发布到其他目录下，以保持源码目录的干净。

fis系统的强大之处在于当你调整了部署规范之后，fis会识别所有资源定位标记，将他们修改为对应的部署结果。

![资源定位替换](https://raw.githubusercontent.com/fouber/blog/master/assets/uri.png)

fis的文件系统设计决定了配置项目规范的成本非常小。fis的构建核心有三个超级正则，用于识别资源定位标记，把用户的开发规范和部署规范通过配置完整连接起来，具体实现可以看[这里](https://github.com/fex-team/fis-kernel/blob/master/lib/compile.js#L376-L465)。
...
好了，我们已经基本解决了开发和部署直接的目录规范问题，这里我需要加快一些步伐，把其他目录的部署规范也配置好，得到一个相对比较完整的结果：
...
从前面的例子可以看出，项目代码使用相对路径即可，fis在构建时会根据fis-conf.js中的配置完成开发路径到部署路径的转换工作。这意味着在fis体系下开发的模块具有非常好的可移植性，既能满足不同项目的不同部署需求，又能允许开发中使用相对路径进行资源定位，工程师再不用把部署路径写到代了。
...
昨天主要解决了开发概念、开发目录规范、部署目录规范以及初步的fis-conf.js配置。今天要进行前端开发体系设计的关键任务——js模块化框架。

> 模块化框架是开发体系中最为核心的环节。

模块化框架肩负着模块管理、资源加载、性能优化（按需，请求合并）等多种重要职责，同时它也是组件开发的基础框架，因此对模块化框架的设计直接影响到开发体系的设计质量。

很遗憾的说，现在市面上已有的模块化框架都没能很好的处理模块管理、资源加载和性能优化三者之间的关系。这倒不是框架设计的问题，而是前端领域语言特殊性决定的。框架设计者一般在思考模块化框架时，通常站在纯前端运行环境角度考虑。因此大多数功能是借助原生js实现的，因此在个别性能优化的关键点上无法很好的解决。这个关键点就是``依赖声明``。

以 [seajs](http://seajs.org/) 为例（无意冒犯），seajs采用运行时分析的方式实现的依赖声明识别，并为依赖关系做进一步的加载处理。比如如下代码：
...
第一个问题还好，差不多多少字节，尤其是在gzip下，不过要配置js压缩器保留require函数不压缩。第二个问题就比较麻烦了，虽然seajs有seajs-combo插件，但仍然不能很好的解决这个问题。举个例子，有如下seajs模块依赖关系树：
...
3. a.js和b.js加载完成后又进一步分析源码，才发现还依赖了c.js、d.js和e.js，再发起combo加载这三个文件
4. 完成c、d、e的加载之后，再分析，发现f.js依赖，再请求
5. 完成f.js的请求，page.js的依赖全部满足，执行它的factory函数。

虽然combo可以在依赖层级上进行合并，但完成page.js的请求仍需要4个。很多团队在使用seajs的时候，为了避免这样的多次依赖分析请求问题，会自己实现打包方案，将所有文件直接打包在一起，放弃了模块化的按需加载能力，也是一种无奈之举。

原因很简单

> 以纯前端方式来实现模块依赖加载不能同时解决性能优化问题。

归根结底，这样的结论是由前端领域语言的特点决定的。前端语言均缺少三种编译能力，前面讲目录规范和部署规范时其实已经提到了一种能力，就是“资源定位”，让工程师使用开发路径定位资源，编译后要转换为部署路径。很多传统软件没有web这种物理上分离的资源部署策略，但也具都有类似'getResource(path)'这样的函数，用于在运行环境下定位当初的开发资源，这样不管项目怎么部署，只要getResource函数运行正常就行了。可惜前端语言没有这样的接口，而只有url这样的资源定位符，它指向的其实并不是开发路径，而是部署路径，这里可以简单列举出前端语言缺少三种的语言能力：
...
根据这样的设计思路，我请 [@hinc](https://github.com/hinc) 帮忙实现了这样一个框架，我告诉他，deps里不但会有js，还会有css，所以也要兼容一下。hinc果然是执行能力非常强的小伙伴，仅一个下午的时间就搞定了框架的实现，我们给这个框架取名为 [scrat.js](https://github.com/scrat-team/scrat.js/blob/master/scrat.js)，仅有393行。

前面提到fis具有资源依赖声明的编译能力。因此只要工程师按照fis的资源依赖方式在代码中声明依赖关系，就能在构建的最后阶段自动获得fis系统整理好的依赖树，然后对依赖的数据结构进行调整、输出，满足框架要求就搞定了！fis规定的资源依赖声明方式为：[在html中声明依赖](https://github.com/fex-team/fis/wiki/%E5%9C%A8html%E4%B8%AD%E5%A3%B0%E6%98%8E%E4%BE%9D%E8%B5%96)，[在js中声明依赖](https://github.com/fex-team/fis/wiki/%E5%9C%A8js%E4%B8%AD%E5%A3%B0%E6%98%8E%E4%BE%9D%E8%B5%96)，[在css中声明依赖](https://github.com/fex-team/fis/wiki/%E5%9C%A8css%E4%B8%AD%E5%A3%B0%E6%98%8E%E4%BE%9D%E8%B5%96)。

接下来，我要写一个配置，生成依赖关系表。fis构建是分流程的，具体构建流程可以看[这里](https://github.com/fex-team/fis/wiki/%E8%BF%90%E8%A1%8C%E5%8E%9F%E7%90%86)。fis会在``postpackager``阶段创建好完整的依赖树表，我就在这个时候写一个插件来处理依赖树即可。
...
{
    deps: {
        'components/bar/bar.js': [ 'components/bar/bar.css' ],
        'components/foo/foo.js': [ 'components/bar/bar.js', 'components/foo/foo.css' ]
    }
}
```

可以看到js也和同名的css自动建立了依赖关系，这是fis默认进行的依赖声明。有了这个表，我们就可以把它打在页面上了。我们为页面准备一个替换用的钩子，比如约定为``__FRAMEWORK_CONFIG__``，这样用户就可以根据需要在合适的地方获取框架配置了。模块化框架的配置一般都是写在非模块化文件中的，比如html页面里，因此，我们应该只针对views目录下的文件做这样的替换就可以。所以我们需要给views下的文件进行一个标记，只有views下的html或js文件才需要进行框架配置写入，具体的配置为：
...
执行 ``fis release -d ../release`` 之后，得到``release/public/proj/1.0.3/index.html``的内容为：
...
接下来还有一个问题，就是模块名太长，开发中写这么长的模块名非常麻烦。我们可以借鉴现行的模块化框架常用的缩短模块名手段——别名(alias)——来降低开发中的模块引用成本。此外，目前的配置其实会针对所有文件生成依赖关系表，我们的开发概念定义只有components和component_modules目录下的文件才是模块化的，因此我们可以进一步的对文件进行分类，这样配置规范：
...
这样，所有在components目录和component_modules目录下的js文件都会被包裹define，并自动根据roadmap.path中的id配置进行模块定义了。

最煎熬的一天终于过去了，睡一觉，拥抱一下周末。

## 2014年02月15日 - 超晴

周末的天气非常好哇，一觉睡到中午才起，这么好的天气。。。
...
这里面有很多需求是fis天生支持的，比如图片的base64内嵌，fis都是先经过编译流程，得到压缩后的图片再做内嵌处理的，内嵌支持任意文件的内嵌，因此，这个语言能力可以同时解决前端模板和图片base64内嵌的需求，比如我们有这样的代码：
...
> 构建工具不需要那么多命令，或者develop、release等不同状态的配置文件，应该从命令行切换编译参数，从而实现开发/上线模式的切换。

另外，由于fis是命令行工具，各种内置的插件也是完全独立无依赖的，可以与ci平台直接对接，在各个主流操作系统下运行正常。

利用fis的内置的各种编译功能，我们离目标又进了许多：
...
那么执行 ``fis say`` 这个命令，就能调用到那个fis-command-say插件了。比如剩下的这个component模块安装，我就利用了这个扩展点，结合component开源的 [component-installer](https://www.npmjs.org/package/component-installer) 包，我就可以把component整合到fis系统下了，这里我们需要创建一个npm包来提供扩展，而不能直接在fis-conf.js中扩展命令行，插件代码我就不贴了，可以看 [这里](https://github.com/fouber/scrat-command-install/blob/master/index.js)。
...
如果团队用起来，会有很多问题，首先，安装过程太过麻烦，其次如果项目多，那么fis-conf.js不能同步升级，这是非常严重的问题。grunt的gruntfile.js也是如此。如果说有一个项目用了某套grunt配置感觉很爽，那么下个项目也想用这套方案，那么复制gruntfile.js是必须的操作，项目用的多了，同步gruntfile的成本就变得非常高了。

因此，fis提供了一种“包装”的能力，它允许你将fis作为内核，包装出一个新的命令行工具，这个工具内置了一些fis的配置，并且把所有命令行调用的参数传递给fis内核去处理。

我们的准备把这套系统打包为一个新的工具，比如我给它取名为 ``scrat``，也是一直松鼠。这个新工具的目录结构是这样的：
...
将这样一个npm包发布出来，你就有了一个特定规范的开发工具，这个工具可以解决前面说的13项技术问题，并提供一套完整的集成解决方案，而你的团队使用的时候，只有两个步骤：

1. 安装这个工具，npm install -g scrat
2. 项目配置只有两项，只徐配置name和version这两个参数
...
如之前一篇 [文章](https://github.com/fouber/blog/issues/1) 所讲，前端集成解决方案有8项技术要素，除了组件仓库，其他7项对于企业级前端团队来说，应该都需要完整实现的。即便暂时不想实现，也会随着业务发展而慢慢完善，这个完善过程是普适的。
...
但正是由于前端语言的这种灵活性，使得前端项目规模在达到一定程度后工程问题成为发展瓶颈，各种技术要素开始彼此出现关联，要用模块化开发，就必须对应某个模块化框架，用这个框架就必须对应某个构建工具，要用这个工具，就必须对应某个包管理工具……这个时候，前端集成解决方案就成了不二选择。

> 当前端项目达到一定规模后，工程问题将成为主要瓶颈，原来孤立的技术要素开始彼此产生影响，需要有人从比较高的角度去梳理适合自己团队的集成解决方案。
